            ...
            try:
                ...
                for j in range(len(_init.register_commands)):
                    try:  # 进行简单的检验，避免规则书问题导致程序崩溃（虽然如果前面就报错了就没用了）
                        ...
                        # 检查返回内容是否合规
                        if isinstance(check_class, response_classes.response_with_num) or isinstance(check_class,
                                                                                                     response_classes.response_with_num_and_success) or isinstance(
                            check_class, response_classes.response_with_success) or isinstance(check_class,
                        ...
                        if _init.register_commands[j] in command_reg:
                            ...
                            # 规则书间指令冲突的提示信息
                            print(outputs["command_confliction"] % (
                                _init.full_name, _init.register_commands[j], conflicted_with.full_name))
                        else:
                            command_reg[
                                _init.register_commands[j]] = judgement_module  # 把指令关键字所对应的规则书记录到command_reg小本本上
                    except:
                        print(outputs["judgement_module_error"] % (_init.full_name, _init.register_commands[j]))
            except FileNotFoundError:
                pass
    # 完成规则书初始化，返回处理过的结果
    gv._set("judgement_modules_map", judgement_modules_map)
    gv._set("command_reg", command_reg)
